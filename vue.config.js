...
/**
 * 环境变量
 * 1，开发（本机）环境：npm run dev=>本地开发服务器 =》设置环境变量：.env.development
 * 2. 生产（线上）环境：npm run build:prod=>线上花钱买服务器=》设置环境变: .env.production
 */
console.log('环境变量设置:', process.env)
...
module.exports = {
  ...
  devServer: {
    ...
    proxy: {
      ...
      // 接口域名发生变化 其余地址保持不变
      '/api': {
        target: 'http://8.142.65.7:3600', // 我们要代理的真实接口地址
